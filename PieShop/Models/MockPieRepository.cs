...
namespace PieShop.Models
{
    public class MockPieRepository : IPieRepository
    {
        ...
        public IEnumerable<Pie> PiesOfTheWeek
        {
            get
            {
                return Pies.Where(p => p.Name.Contains("Pie"));
        ...
        public Pie GetPieById(int pieId)
        {
            ...
            var pies = Pies.Where(p => p.PieId == pieId);

            if (pies == null)
            {
                throw new ArgumentOutOfRangeException("No pies found with that Id");
            }
            else return pies.Single();
        }
    }
}
